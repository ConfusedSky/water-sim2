...
void main() {
    ...
    // Continuous projected sphere diameter in pixels.
    float eye_neg    = -pos_eye.z;
    float diameter   = u_particle_radius * u_proj[1][1] * u_viewport_h / eye_neg;

    // Pad the rasterized sprite by a couple of pixels so the disc boundary
    // computed in the fragment shader (against the *continuous* diameter)
    // never coincides with the sprite's integer edge.
    float sprite_size = ceil(diameter) + 2.0;
    gl_PointSize      = max(1.0, sprite_size);

    // Normalized sphere radius inside the sprite's [-1, 1] coords.
