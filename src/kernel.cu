...
__device__ inline int3 cell_coords_for(float3 p) {
  ...
  int cz =
      static_cast<int>(floorf((p.z - c_params.box_min.z) / c_params.cell_size));
...
__global__ void apply_mouse_force_kernel(const float3 *pos, float3 *vel, int n,
                                         float dt, MouseState mouse) {
  if (mouse.button_state == kMouseButtonNone)
    return;
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
...
__global__ void reorder_positions_kernel(const float3 *positions,
                                         const int *particle_index,
                                         float3 *pos_sorted, int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n)
    return;
  pos_sorted[idx] = positions[particle_index[idx]];
}
    ...
    for (int dy = -1; dy <= 1; ++dy) {
      ...
      for (int dx = -1; dx <= 1; ++dx) {
        ...
        for (int idx = start; idx < end; ++idx) {
          float3 pj = pos_sorted[idx];
          if (length2(pi - pj) >= h2)
            continue;
          if (particle_index[idx] == i)
            continue;
          if (cnt < kMaxNeighbors)
            neighbors[cnt++ * n + i] = idx;
        }
      }
    }
  }
...
__global__ void compute_lambda_kernel(const float3 *positions,
  ...
  for (int k = 0; k < cnt; ++k) {
    ...
    float3 rij = pi - pos_sorted[idx];
    float3 gw = spiky_gradient(rij);
    float3 grad_j = gw / c_params.rest_density;
    rho += poly6_weight(rij);
...
compute_delta_p_kernel(const float3 *positions, const float3 *pos_sorted,
  ...
  for (int k = 0; k < cnt; ++k) {
    ...
    delta += spiky_gradient(rij) * (lambda_i + lambda[j] + s_corr);
  }
...
__global__ void enforce_box_boundary_kernel(float3 *pos, float3 *vel, int n,
                                            int reflect) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
...
#define CLAMP_AXIS(axis, min_val, max_val)                                     \
  if (p.axis < (min_val) + r) {                                                \
    p.axis = (min_val) + r;                                                    \
    if (reflect && v.axis < 0.0f)                                              \
      v.axis *= -c_params.boundary_bounce;                                     \
...
__global__ void export_render_particles_kernel(const float3 *pos,
  ...
  if (i >= n)
    return;
  render_particles[i] = make_float4(pos[i].x, pos[i].y, pos[i].z,
...
void compute_grid_dims(SimParams &p) {
  ...
  p.grid_d = std::max(
      1, static_cast<int>(std::ceil((p.box_max.z - p.box_min.z) / cs)));
}

void allocate_simulation_buffers() {
  CUDA_CHECK(cudaMalloc(&g_pos, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_vel, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_vel_scratch, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_predicted, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_pos_sorted, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_delta_p, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_vorticity, kSceneParticleCount * sizeof(float3)));
  CUDA_CHECK(cudaMalloc(&g_lambda, kSceneParticleCount * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&g_density, kSceneParticleCount * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&g_neighbors,
                        kSceneParticleCount * kMaxNeighbors * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&g_neighbor_counts, kSceneParticleCount * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&g_cell_hash, kSceneParticleCount * sizeof(int)));
  CUDA_CHECK(cudaMalloc(&g_particle_index, kSceneParticleCount * sizeof(int)));
...
float estimate_rest_density(float kernel_radius) {
  ...
  int sample_count = std::min(g_particle_count, kMaxSamples);
  int stride = std::max(1, g_particle_count / sample_count);

  std::vector<float> densities;
  densities.reserve(sample_count);
  for (int i = 0; i < g_particle_count && (int)densities.size() < sample_count;
       i += stride) {
    float rho = host_poly6_weight(make_float3(0.0f, 0.0f, 0.0f), kernel_radius);
    for (int j = 0; j < g_particle_count; ++j) {
      if (i == j)
        continue;
...
void upload_params() {
  ...
  g_params.viscosity_c = 0.000000f;
...
void rebuild_spatial_grid(const float3 *positions, float3 *pos_sorted,
                          int blocks, int num_cells) {
  ...
  thrust::sort_by_key(thrust::device, g_cell_hash,
                      g_cell_hash + g_particle_count, g_particle_index);
  ...
  reorder_positions_kernel<<<blocks, kThreadsPerBlock>>>(
      positions, g_particle_index, pos_sorted, g_particle_count);
}
...
void run_post_step_passes(int blocks, float dt) {
  ...
  if (g_params.viscosity_c > 0.0f) {
    CUDA_CHECK(cudaMemcpyAsync(g_vel_scratch, g_vel,
                               g_particle_count * sizeof(float3),
...
void init_simulation() {
  ...
  g_particle_count = static_cast<int>(g_initial_positions.size());
  upload_params();
...
void reset_simulation() {
  if (!g_pos)
    return;
  std::vector<float3> zero_vel(g_particle_count, make_float3(0.0f, 0.0f, 0.0f));
  ...
  CUDA_CHECK(cudaMemcpy(g_vel, zero_vel.data(),
                        g_particle_count * sizeof(float3),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemset(g_delta_p, 0, g_particle_count * sizeof(float3)));
  CUDA_CHECK(cudaMemset(g_vorticity, 0, g_particle_count * sizeof(float3)));
  ...
  CUDA_CHECK(cudaMemset(g_neighbors, 0,
                        g_particle_count * kMaxNeighbors * sizeof(int)));
  CUDA_CHECK(cudaMemset(g_stats, 0, sizeof(DeviceStats)));
  g_last_stats = {};
}
...
void shutdown_simulation() {
  ...
  cudaFree(g_pos);
  cudaFree(g_vel);
  cudaFree(g_vel_scratch);
  cudaFree(g_predicted);
  cudaFree(g_pos_sorted);
  cudaFree(g_delta_p);
  cudaFree(g_vorticity);
  cudaFree(g_lambda);
  cudaFree(g_density);
  cudaFree(g_neighbors);
  cudaFree(g_neighbor_counts);
  cudaFree(g_cell_hash);
  cudaFree(g_particle_index);
  cudaFree(g_cell_start);
  cudaFree(g_cell_end);
  cudaFree(g_stats);
  g_pos = nullptr;
  g_vel = nullptr;
...
void set_tunable_params(const TunableParams &p) {
  ...
  g_params.velocity_damping =
      std::max(0.0f, std::min(p.velocity_damping, 1.0f));
  ...
  g_params.max_position_correction =
      std::max(p.max_position_correction, 1.0e-5f);
  compute_grid_dims(g_params);
  CUDA_CHECK(cudaMemcpyToSymbol(c_params, &g_params, sizeof(g_params)));
}
...
void step_simulation(float dt, const MouseState &mouse,
  ...
  for (int iter = 0; iter < g_params.solver_iterations; ++iter) {
    rebuild_spatial_grid(g_predicted, g_pos_sorted, blocks, num_cells);
    find_neighbors_grid_kernel<<<blocks, kThreadsPerBlock>>>(
        g_predicted, g_pos_sorted, g_particle_index, g_cell_start, g_cell_end,
        g_neighbors, g_neighbor_counts, g_particle_count);
    compute_lambda_kernel<<<blocks, kThreadsPerBlock>>>(
        g_predicted, g_pos_sorted, g_neighbors, g_neighbor_counts, g_lambda,
        g_density, g_particle_count);
    compute_delta_p_kernel<<<blocks, kThreadsPerBlock>>>(
        g_predicted, g_pos_sorted, g_particle_index, g_neighbors,
        g_neighbor_counts, g_lambda, g_delta_p, g_particle_count);
